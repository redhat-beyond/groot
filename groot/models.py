...
class User(db.Model):
    ...
    def __repr__(self):
        return {'email':self.email, 'first_name':self.first_name,'last_name':self.last_name,
         'nick_name':self.nick_name,'hash_pw':self.hash_pw}
...
class Policy(db.Model):
    ...
    date_created = db.Column(db.DateTime, nullable=False, default=datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S"))
...
class Sensor(db.Model):
    ...
    def __repr__(self):
        return {'id':self.id, 'name':self.name,'type':self.type, 'active':self.active}
...
class Comment(db.Model):
    ...
    date_posted = db.Column(db.String(30), nullable=False, unique=True, default=datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S"))
    writer = db.Column(db.Integer, db.ForeignKey('users.id'), nullable=False)
